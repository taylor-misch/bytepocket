...
function createHTTPServer(options, httpPort) {
    ...
    function applyPromiseWithCallback(fnName, callback, args_) {
        try {
            let args = args_;
            if (!_.isArray(args_)) {
                const fnParameters = jayson_1.default.Utils.getParameterNames(rippleAPI[fnName]);
    ...
    _.forEach(methodNames, fn => {
        methods[fn] = jayson_1.default.Method((args, cb) => {
            applyPromiseWithCallback(fn, cb, args);
        }, { collect: true });
    });
    const server = jayson_1.default.server(methods);
    let httpServer = null;
    return {
        server: server,
        start: function () {
